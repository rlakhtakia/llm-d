...
EPP Flow Control addresses this by shifting queuing to the gateway. The EPP's `llmd_inference_scheduler_flow_control_queue_size` metric directly counts requests that the current pool cannot absorb — a precise, actionable signal for scale-out.
...
1. **Metric Emission**: The EPP exposes `llmd_inference_scheduler_flow_control_queue_size` (requests buffered in Flow Control) and `llmd_inference_scheduler_running_requests` (active in-flight requests) on its `/metrics` endpoint.
