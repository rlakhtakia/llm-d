...
* **TPU v7x:** Your GKE cluster must have TPU 7x nodes provisioned with a `2x2x1` topology (4 chips per node) to accommodate the model requirements.
  > [!NOTE]
  > **TPU Cores and Parallelism:** TPU7x has 2 cores per chip. With 4 chips per pod, the tensor parallel size (`--tensor-parallel-size`) is set to `8` in `guides/pd-disaggregation/modelserver/tpu/vllm/patch-decode.yaml`.
